<div>
    <div class="card">
        <div class="card-header">
            ...
            <div class="card-tools">
                @can('add balita')
                <a name="tambahBalita" id="tambahBalita" class="btn btn-sm btn-primary"
                    href="{{ url('/balita/tambah') }}" role="button">Tambah</a>
                @endcan
            </div>
        </div>
        <div class="card-body">
                    ...
                    <div class="col-md-6">
                        <table class="table">
                            <tbody>
                                <tr>
                                    <td
                                        class="@if(str_contains($hasil->bb_u, 'kurang')) text-danger @else text-dark @endif ">
                                        {{ $hasil->bb_u }}</td>
                                    <td>
                                        : {{ $this->hasilBBU($hasil->bb_u) }}
                                        @if(str_contains($hasil->bb_u, 'kurang'))
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal"
                                            data-target="#konsulGiziModal">
                                ...
                                <tr>
                                    <td
                                        class="@if(str_contains($hasil->tb_u, 'pendek') || str_contains($hasil->tb_u, 'Pendek')) text-danger @else text-dark @endif ">
                                        {{ $hasil->tb_u }}</td>
                                    <td>
                                        : {{ $this->hasilTBU($hasil->tb_u) }}
                                        @if(str_contains($hasil->tb_u, 'pendek') || str_contains($hasil->tb_u,
                                        'Pendek'))
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal"
                                            data-target="#konsulModal">
                                ...
                                <tr>
                                    <td
                                        class="@if(str_contains($hasil->bb_tb, 'buruk') || str_contains($hasil->bb_tb, 'Buruk') || str_contains($hasil->bb_tb, 'kurang')) text-danger @else text-dark @endif ">
                                        {{ $hasil->bb_tb }}</td>
                                    <td> : {{ $this->hasilBBTB($hasil->bb_tb) }} </td>
                                    @if(str_contains($hasil->bb_tb, 'buruk') || str_contains($hasil->bb_tb, 'Buruk'))
                                    <button type="button" class="btn btn-success btn-sm" data-toggle="modal"
                                        data-target="#konsulGiziModall">
                    ...
                    <div class="col-md-6">
                        <form wire:submit.prevent='kirimCatatanKonsul'>
                            <div class="form-group m-4">
                                ...
                                <textarea class="form-control" wire:model.defer='catatanKonsul' name="catatan_konsul"
                                    id="catatan_konsul" rows="3"></textarea>
        ...
        {{-- <div class="card-footer">
            <div class="d-flex justify-content-end">
                <a name="tambahBalita" id="tambahBalita" class="btn btn-primary" href="{{ url('/balita/tambah') }}"
    ...
    <!-- Modal -->
    <div class="modal fade" id="konsulModal" tabindex="-1" role="dialog" aria-labelledby="konsulModalLabel"
        aria-hidden="true">
    ...
    <!-- Modal -->
    <div class="modal fade" id="konsulGiziModal" tabindex="-1" role="dialog" aria-labelledby="konsulGiziModalLabel"
        aria-hidden="true">
